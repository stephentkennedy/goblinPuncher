...
function stats(){
	$('#info').html('Name: ' +hero.fName +" "+ hero.lName+ ' HP: '+hero.curHP+'/'+hero.totalHP+' Str: '+hero.str+' Dex: '+hero.dex+' Con: '+hero.con+' Int: '+hero.int+' Wis: ' +hero.wis+' Cha: '+hero.cha);
}
var hero = {};
var monster = {};
...
function characterCreate(object, level){
	if(level == undefined){
		level = 1;
	}
	object.fName = ranArray(namesFirst);
	object.lName = ranArray(namesLast);
	object.gender = ranArray(genders);
	object.race = ranArray(races);
	/*object.str = rand(10,18);
	object.dex = rand(10,18);
	object.con = rand(10,18);
	object.int = rand(10,18);
	object.wis = rand(10,18);
	object.cha = rand(10,18);*/
	object.str = 10;
	object.dex = 10;
	object.con = 10;
	object.int = 10;
	...
	object.ac = 10 + getBonus(object.dex);
	object.touch = 10;
	object.fFoot = 10;
	object.fort = rand(1,6);
	object.reflex = rand(1,6);
	object.will = rand(1,6);
	object.aBonus = rand(1,3);
	object.cmb = object.aBonus + getBonus(object.str);
	object.cmd = 10 + object.aBonus + getBonus(object.str);
	object.totalHP = rollStack(rand(4,10), 1) + getBonus(object.con);
	object.curHP = object.totalHP;
	object.i = getBonus(object.dex);
}
function attack(){
	var hit = rollStack(20, 1) + hero.str;
	if (hit > monster.ac){
		var damage = rollStack(4,1);
		monster.curHP = monster.curHP - damage;
		var output = ranArray(heroUnAtkMsg) + monster.fName +", doing " + bad(damage) + " points of damage.<br>";
		if(monster.curHP <= 0){
			output += monster.fName + " is Dead.<br>";
			menuC();
		}
	}else{
		var output = "You miss.<br>";
	}
	o(output);
}
function defend(){
	var hit = rollStack(20, 1) + monster.str;
	if (hit > hero.ac){
		var damage = rollStack(4,1);
		hero.curHP = hero.curHP - damage;
		var output = monster.fName + ranArray(monAtkMsg) + ", doing " + bad(damage) + " points of damage.<br>";
		if(hero.curHP <= 0){
			output += ranArray(deathMsg)+"<br>";
			menuC();
		}
	}else{
		var output = monster.fName +" misses.<br>";
	}
	o(output);
	stats();
}
function battleRound(){
	var heroI = hero.i + rollStack(20,1);
	var monI = monster.i + rollStack(20,1);
	if (heroI >= monI){
		attack();
		if(monster.curHP > 0){
			setTimeout(defend, 500);
		}
	}else{
		defend();
		if(hero.curHP > 0){
...
$(document).ready(
	function(){
		characterCreate(hero);
		characterCreate(monster);
		stats();
		o("Vs.<br>" + monster.fName + " " + monster.lName + "<br>");
		menu("<button class='button' id='attack'>Attack!</button>");
